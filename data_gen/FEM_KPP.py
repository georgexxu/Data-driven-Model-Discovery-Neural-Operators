import os

from fenics import *
import numpy as np
from scipy.io import loadmat

set_log_level(LogLevel.ERROR)

u0_all = loadmat('./data/data_Dirichlet.mat')['input']
u0_all *= 100
T = 1.0
num_steps = 100
dt = T / num_steps
kappa = 1.0
lam = 1.0
h = 1/1024
N = int(1/h)
mesh = UnitIntervalMesh(N)
V = FunctionSpace(mesh, 'P', 1)
u_D = Constant(0.)
bc = [DirichletBC(V, u_D, "on_boundary")]
u_0 = Function(V)
results = np.zeros([u0_all.shape[0], num_steps + 1, N + 1])

num_samples = u0_all.shape[0]
total_solves = num_samples * num_steps
done = 0

for i in range(num_samples):
    print(f"Sample {i + 1}/{num_samples}", flush=True)
    u_0.vector()[:] = np.flip(u0_all[i, :]).astype(u_0.vector()[:].dtype)

    u = TrialFunction(V)
    u_n = Function(V)
    u_n.assign(u_0)

    v = TestFunction(V)
    f = lam * u_n * (kappa - u_n)

    a = u / dt * v * dx + dot(grad(u), grad(v)) * dx
    F = f * v * dx + u_n / dt * v * dx
    results[i, 0, :] = np.array(np.flip(u_n.vector()[:]))

    for n in range(num_steps):
        u_sol = Function(V)
        solve(a == F, u_sol, bc)
        u_n.assign(u_sol)
        results[i, n + 1, :] = np.array(np.flip(u_n.vector()[:]))
        done += 1
        if done % 100 == 0 or done == total_solves:
            pct = 100.0 * done / total_solves
            print(f"  {done}/{total_solves} solves ({pct:.1f}%)", flush=True)
np.save('./data/data_kpp.npy', results)
print("Saved ./data/data_kpp.npy", flush=True)
os._exit(0)
